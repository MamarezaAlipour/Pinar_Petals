...
import pinar_petals.hazard.emulator.const as const
...
class HazRegion():
    ...
    def centroids(self, latlon=None, res_as=360):
        ...
        latlon : pair (lat, lon), optional
            ...
            If not given, values are taken from PINAR's base grid (see `res_as`).
        res_as : int, optional
            One of 150 or 360. When `latlon` is not given, choose coordinates from centroids
            according to PINAR's base grid of given resolution in arc-seconds. Default: 360.

        Returns
        -------
        centroids : pinar.hazard.Centroids object
        """
        if latlon is None:
